...
- ✅ **配置记忆**：自动保存窗口大小、窗口位置、编码参数、最后使用的目录等
...
- **编码参数**：视频编码器、CRF、预设、分辨率、帧率、位深等
- **音频参数**：音频编码器、码率、备用音频编码参数
- **窗口设置**：窗口大小、窗口位置、表格列宽和顺序
...
- **Windows**：主要开发和测试平台，功能完整且稳定。
- **Linux/macOS**：由 GitHub Actions 自动构建，但作者未进行实际测试，可能存在未知问题。使用这些平台版本时请自行承担风险，作者不对这些平台的问题负责。
...
**注意**：打包时会自动包含 `version` 文件，确保程序设置页面能正确显示版本号。
