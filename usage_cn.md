...
- 有两种方式让程序找到 FFmpeg：
  1. **添加到系统环境变量 PATH（推荐）**  
     - 将 FFmpeg 可执行文件所在目录（例如 `C:\ffmpeg\bin`）加入系统环境变量 `PATH`；  
     - 程序会自动从 PATH 中查找 `ffmpeg.exe`。  
  2. **在“设置”中手动指定路径**  
     - 打开工具栏右侧的 **“设置”**；  
     - 在顶部“FFmpeg 设置”里，手动选择 `ffmpeg.exe` 的完整路径；  
     - 保存后，程序会优先使用这个路径。  
- 如果 FFmpeg 既不在 PATH 中，也没有在设置里手动配置，程序会在启动或编码时提示 “FFmpeg 未初始化，请检查设置”。
